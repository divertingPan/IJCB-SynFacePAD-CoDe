...
from utils import performances_cross_db, hypersphere_loss
from model.net import MultiFTNet, EnsembleNet, EnsembleNet_c3
...
if __name__ == '__main__':
    ...
    parser.add_argument("--prefix", default='train_final', type=str, help="log description")
    ...
    for epoch in range(args.max_epoch):
        if os.path.isfile(os.path.join(checkpoint_save_dir, '{}.pth'.format(epoch))):
            ...
            for i, (data_1, data_2) in enumerate(zip(train_loader_1, train_loader_2)):
                ...
                l_hypersphere = (hypersphere_loss(vgg_features_1, label_1, r=0.1, m=10, device=device) +
                                 hypersphere_loss(vgg_features_2, label_2, r=0.1, m=10, device=device) +
                                 hypersphere_loss(a_features_1, label_1, r=0.1, m=10, device=device) +
                                 hypersphere_loss(a_features_2, label_2, r=0.1, m=10, device=device)) * 0.0025
                ...
                loss = loss_bce_cls + loss_ascore_cls + loss_vscore_cls + l_hypersphere - loss_v_feature - loss_a_feature

                """ using hypersphere loss, deprecate these losses """
                ...
                if (i + 1) % args.log_step == 0:
                    ...
                    print(f'Iter [{i + 1}/{num_iter}]'
                          ...
                          f' Loss_hypersphere: {l_hypersphere.item():.4f},'
                          )
