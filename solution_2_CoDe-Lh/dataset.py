...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = np.concatenate((image, image_hsv), axis=-1)
...
class TrainDatasetIdentityDual(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        count = self.dataframe.iloc[idx, :].count() - 2
        rand_idx = random.randint(1, count)
        img_1_path = self.dataframe.iloc[idx, 1]
        img_2_path = self.dataframe.iloc[idx, 1 + rand_idx]
        image_1 = cv2.cvtColor(cv2.imread(img_1_path), cv2.COLOR_BGR2RGB)
        image_2 = cv2.cvtColor(cv2.imread(img_2_path), cv2.COLOR_BGR2RGB)
        label_1 = 1 if 'BonaFide' in img_1_path else 0
        label_2 = 1 if 'BonaFide' in img_2_path else 0

        image_1 = self.composed_transformations(image=image_1)['image']
        image_2 = self.composed_transformations(image=image_2)['image']
